...
OQS_API OQS_STATUS OQS_KEM_fptru_1277_keypair(uint8_t *public_key,uint8_t *secret_key) {
    return (OQS_STATUS) crypto_kem_keygen(public_key ,secret_key); //TODO:这里的强制类型转化可能会存在问题  根据虚拟机上的结果修改一下->经过测试，加不加的结果都是不匹配(sad)
}

OQS_API OQS_STATUS OQS_KEM_fptru_1277_encaps(uint8_t *ciphertext,uint8_t *shared_secret,const uint8_t *public_key) {
    return (OQS_STATUS) crypto_kem_encaps(ciphertext,shared_secret,public_key);
}

OQS_API OQS_STATUS OQS_KEM_fptru_1277_decaps(uint8_t *shared_secret,const uint8_t *ciphertext,const uint8_t *secret_key) {
