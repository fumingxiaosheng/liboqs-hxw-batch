...
*                            (of length OSKR_POLYCOMPRESSEDBYTES)
*              - const poly *a: pointer to input polynomial
**************************************************/
void poly_compress_3(uint8_t r[OSKR_POLYCOMPRESSEDBYTES_CV], poly *a) {
    unsigned int i, j;
    uint8_t t[8];
    poly_caddq(a);
    ...
    for (i = 0; i < OSKR_N / 8; i++) {
        for (j = 0; j < 8; j++) {
            t[j] = ((((uint32_t)a->coeffs[8*i+j] << 3) + q/2) / q) & 7;
        }
    ...
    t[0] = ((((uint32_t)a->coeffs[568] << 3) + q/2) / q) & 7;
    t[1] = ((((uint32_t)a->coeffs[569] << 3) + q/2) / q) & 7;
    t[2] = ((((uint32_t)a->coeffs[570] << 3) + q/2) / q) & 7;
    r[0]  = (t[0] >> 0) | (t[1] << 3) | (t[2] << 6);
    r[1]  = (t[2] >> 2);
}

void poly_compress_11(uint8_t r[OSKR_POLYCOMPRESSEDBYTES_CU], poly *a) {
    unsigned int i, j;
    uint16_t t[8];
    poly_caddq(a);
    ...
    t[0] = ((((uint32_t)a->coeffs[568] << 11) + q/2) / q) & 0x7ff;
    t[1] = ((((uint32_t)a->coeffs[569] << 11) + q/2) / q) & 0x7ff;
    t[2] = ((((uint32_t)a->coeffs[570] << 11) + q/2) / q) & 0x7ff;
    r[0] = (t[0] >> 0);
    r[1] = (t[0] >> 8) | (t[1] << 3);
    r[2] = (t[1] >> 5) | (t[2] << 6);
    r[3] = (t[2] >> 2);
...
*                                  (of length OSKR_POLYCOMPRESSEDBYTES bytes)
**************************************************/
void poly_decompress_3(poly *r, const uint8_t a[OSKR_POLYCOMPRESSEDBYTES_CV]) {
    unsigned int i,j;
    uint8_t t[8];
    ...
    for (i = 0; i < OSKR_N / 8; i++) {
        ...
        for (j = 0; j < 8; j++)
            r->coeffs[8 * i + j] = ((uint32_t) (t[j] & 7) * q + 4) >> 3;
    }
    t[0] = (a[0] >> 0);
    t[1] = (a[0] >> 3);
    t[2] = ((a[0] >> 6) | (a[1] << 2));
    r->coeffs[568] = ((uint32_t) (t[0] & 7) * q + 4) >> 3;
    r->coeffs[569] = ((uint32_t) (t[1] & 7) * q + 4) >> 3;
    r->coeffs[570] = ((uint32_t) (t[2] & 7) * q + 4) >> 3;
}

void poly_decompress_11(poly *r, const uint8_t a[OSKR_POLYCOMPRESSEDBYTES_CU]) {
    unsigned int i, j;
    uint16_t t[8];
    for (i = 0; i < OSKR_N / 8; i++) {
    ...
    t[1] = (a[1] >> 3) | ((uint16_t) a[2] << 5);
    t[2] = (a[2] >> 6) | ((uint16_t) a[3] << 2) | ((uint16_t) a[4] << 10);
    r->coeffs[568] = ((uint32_t) (t[0] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[569] = ((uint32_t) (t[1] & 0x7FF) * q + 1024) >> 11;
    r->coeffs[570] = ((uint32_t) (t[2] & 0x7FF) * q + 1024) >> 11;
}
...
void poly_frombytes(poly *r, const uint8_t a[OSKR_POLYBYTES]) {
    ...
    t[1] = (a[1] >> 5) | ((uint16_t) a[2] << 3) | ((uint16_t) a[3] << 11);
    t[2] = (a[3] >> 2) | ((uint16_t) a[4] << 6); 
    r->coeffs[568] = t[0] & 0x1FFF;
    r->coeffs[569] = t[1] & 0x1FFF;
    r->coeffs[570] = t[2] & 0x1FFF;
}
/*************************************************
* Name:        polyeta_tobytes
*
* Description: Serialization of a sk
*
* Arguments:   - uint8_t *r: pointer to output byte array
*                            (needs space for OSKR_POLYBYTES bytes)
*              - const poly *a: pointer to input polynomial
...
void polyeta_tobytes(uint8_t r[OSKR_SECRETBYTES], const poly *a) {
    ...
    t[0] = OSKR_ETA1 - a->coeffs[568];
    t[1] = OSKR_ETA1 - a->coeffs[569];
    t[2] = OSKR_ETA1 - a->coeffs[570];
...
*
* Description: De-serialization of a polyeta;
...
void polyeta_frombytes(poly *r, const uint8_t a[OSKR_SECRETBYTES]) {
    ...
    r->coeffs[568] = OSKR_ETA1 - t[0];
    r->coeffs[569] = OSKR_ETA1 - t[1];
    r->coeffs[570] = OSKR_ETA1 - t[2];
...
void poly_frommsg(poly *r, const uint8_t msg[OSKR_INDCPA_MSGBYTES]) {
    ...
    for (i = 0; i < 64; i++) {
        for (j = 0; j < 8; j++) {
            mask = -(int16_t) ((msg[i] >> j) & 1);// 0 or -1
            r->coeffs[8 * i + j] = mask & ((q + 1) / 2);// 0 or q/2
        }
    }
    for (i = 512; i < 571; i++) 
        r->coeffs[i] = 0; 
...
void poly_tomsg(uint8_t msg[OSKR_INDCPA_MSGBYTES], const poly *a) {
    ...
    for (i = 0; i < 64; i++) {
        ...
        for (j = 0; j < 8; j++) {
            t = a->coeffs[8 * i + j];
            t += ((int16_t) t >> 15) & q; // transform to positive
            t = (((t << 1) + q / 2) / q) & 1;
            msg[i] |= t << j;
        }
    }
...
void poly_getnoise_eta1(poly *r, const uint8_t seed[OSKR_SYMBYTES], uint8_t nonce) {
    uint8_t buf[286];// 2η-bit
    prf(buf, sizeof(buf), seed, nonce);// nonce拼接到最后1个字节 seed + nonce
    cbd2(r, buf);
}
...
void poly_con(uint8_t r[OSKR_POLYCOMPRESSEDBYTES_CV], poly *v, const uint8_t m[OSKR_INDCPA_MSGBYTES]) {
    ...
    for (i = 0; i < OSKR_N / 8; i++) {
        for (j = 0; j < 8; j++) {
            t[j] = ((((uint32_t)v->coeffs[8*i+j] << 3) + q/2) / q) & 7;
        }
    ...
    t[0] = ((((uint32_t)v->coeffs[568] << 3) + q/2) / q) & 7;
    t[1] = ((((uint32_t)v->coeffs[569] << 3) + q/2) / q) & 7;
    t[2] = ((((uint32_t)v->coeffs[570] << 3) + q/2) / q) & 7;
    r[0]  = (t[0] >> 0) | (t[1] << 3) | (t[2] << 6);
    r[1]  = (t[2] >> 2);
...
void poly_rec(uint8_t m[OSKR_INDCPA_MSGBYTES], const uint8_t c2[OSKR_POLYCOMPRESSEDBYTES_CV], const poly *su) {
    ...
    for (i = 0; i < 64; i++) {
        m[i] = 0;
        for (j = 0; j < 8; j++) {
              tmp = ((int32_t)c2_coeffs[8 * i + j] + 2) * q - ((int32_t)su->coeffs[8 * i + j] << 3);
              //tmp = (tmp * 37402) >> 30;
              tmp = tmp / 28708 - ((uint64_t)tmp >> 63);
              tmp = tmp & 1;
              m[i] |= tmp << j;
        }
    }
