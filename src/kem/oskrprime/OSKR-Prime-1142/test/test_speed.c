...
static void poly_naivemul(int16_t c[571], const int16_t a[571], const int16_t b[571]) {
  unsigned int i,j;
  int16_t t1[2*571] = {0};
  for(i = 0; i < 571; ++i){
      for(j = 0; j < 571; ++j){
  ...
  c[0] = barrett_reduceq(t1[0] + t1[571]);
  for(i = 1; i < 570; i++)
    c[i] = barrett_reduceq(t1[i + 570] + t1[i + 571] + t1[i]);
  c[570] = barrett_reduceq(t1[570] + t1[1140]);
}
