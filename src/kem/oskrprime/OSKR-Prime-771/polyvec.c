...
static unsigned int rej_uniform(int16_t *r, unsigned int len, const uint8_t *buf, unsigned int buflen)// len为生成系数数量
{
  ...
  ctr = pos = 0;
  while(ctr < len && pos + 3 <= buflen) {// 3bytes -> 2coeffs
...
void gen_matrix(polyvec a[OSKR_K], const uint8_t seed[OSKR_SYMBYTES], int transposed)
{
  ...
  for(i=0;i<OSKR_K;i++) {
    for(j=0;j<OSKR_K;j++) {
      ...
      while(ctr < OSKR_N) {// not enough
        off = buflen % 3;
        for(k = 0; k < off; k++)// 使用多的字节流，放入最前面，共计off个；每次都是用字节流最后off个
          buf[k] = buf[buflen - off + k];
        xof_squeezeblocks(buf + off, 1, &state);// squeeze 1 block
        buflen = off + XOF_BLOCKBYTES;// 字节流+block
        ctr += rej_uniform(a[i].vec[j].coeffs + ctr, OSKR_N - ctr, buf, buflen);
      }
    }
  }
}
