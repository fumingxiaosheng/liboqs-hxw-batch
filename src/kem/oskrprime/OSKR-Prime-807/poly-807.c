...
**************************************************/
void poly_compress_4(uint8_t r[OSKR_POLYCOMPRESSEDBYTES_CV], poly *a) {
    unsigned int i, j;
    uint8_t t[4];
    poly_caddq(a);

    for (i = 0; i < OSKR_N / 4; i++) {
        for (j = 0; j < 4; j++) {
...
**************************************************/
void poly_decompress_4(poly *r, const uint8_t a[OSKR_POLYCOMPRESSEDBYTES_CV]) {
    unsigned int i,j;
    uint8_t t[4];
...
void polyeta_tobytes(uint8_t r[OSKR_SECRETBYTES], const poly *a) {
    ...
    t[0] = OSKR_ETA1 - a->coeffs[264];
    t[1] = OSKR_ETA1 - a->coeffs[265];
    t[2] = OSKR_ETA1 - a->coeffs[266];
    t[3] = OSKR_ETA1 - a->coeffs[267];
    t[4] = OSKR_ETA1 - a->coeffs[268];
...
void polyeta_frombytes(poly *r, const uint8_t a[OSKR_SECRETBYTES]) {
    ...
    r->coeffs[264] = OSKR_ETA1 - t[0];
    r->coeffs[265] = OSKR_ETA1 - t[1];
    r->coeffs[266] = OSKR_ETA1 - t[2];
    r->coeffs[267] = OSKR_ETA1 - t[3];
    r->coeffs[268] = OSKR_ETA1 - t[4];
